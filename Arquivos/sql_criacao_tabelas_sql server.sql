...
where table_name = 'Produtor'; --SCRIPT para se verificar quais colunas possuem uma tabela
...
CREATE TABLE ControleLicenca
(
    Id                  INT IDENTITY(1,1) PRIMARY KEY,
    UltimoDesbloqueio   DATETIME      NOT NULL,
    DataServidorRegistro DATETIME     NOT NULL DEFAULT GETDATE(),
    -- ? usado para detectar se o relógio do PC foi atrasado
    Ativo               BIT           NOT NULL DEFAULT 1
);
GO

-- Insere um registro inicial (necessário para o sistema funcionar
-- na primeira execução, simulando um "desbloqueio" na instalação)
INSERT INTO dbo.ControleLicenca (UltimoDesbloqueio, DataServidorRegistro)
VALUES (GETDATE(), GETDATE());
GO


-- Verifica Tabela, Coluna, TipoDado, TamanhoMaximo, Precisão e Escala do dado
SELECT 
    t.name AS Tabela,
    c.name AS Coluna,
    TYPE_NAME(c.user_type_id) AS TipoDado,
...
WHERE t.name = 'VENDAS'
  AND c.name = 'PAGAMENTO' -- Opcional: remova esta linha para ver todas as colunas
...
INSERT INTO Vendas (Data, Quantidade, Preco_unitario, Preco_total, Cliente, Nome_Produto, Pago)
